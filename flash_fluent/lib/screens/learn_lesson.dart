...
class _LearnLessonState extends State<LearnLesson> {
  ...
  Widget build(BuildContext context) {
    ...
    if (lessonBookmarks.value.contains(lesson)) {
      bookmarked = true;
    }
    void nextPage() {
    ...
    void makeBookmark() {
      lessonBookmarks.value.add(lesson);
    ...
    void removeBookmark() {
      ...
      final updatedList = List<Lesson>.from(lessonBookmarks.value);
      updatedList.removeWhere((item) => item.title == lesson.title);
      lessonBookmarks.value = updatedList;
                ...
                StyledButton(
                  ...
                  func: page == lesson.pages.length - 1
                      ? () async {
                          ...
                          if (!exists) {
                            await _saveService.saveCompletedLesson(
                              lesson.title,
                              true,
                            );
